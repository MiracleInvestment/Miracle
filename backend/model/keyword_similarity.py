...
class Keyword_similarity:
    ...
    def keyword_one_sentence(self, keyword, sentence):
        ...
        keyword_vec = self.model.wv.get_vector(keyword)
        ...
        for word in set(split_sentence):
            ...
            #for split_word in pos_word:             
                ...
                if pos_word[0][1] in ['Noun']:
                    try: 
                        word_vec = self.model.wv.get_vector(pos_word[0][0])
                    except:
                        continue
                    
                    if cosine_similarity(keyword_vec, word_vec) > self.threshold and pos_word[0][0] not in word_list:
                        '''results = re.finditer(split_word[0], sentence)
...
def make_keyword_list(keyword_list, sentence_list):
    model_name = "/home/oceanofglitta/server/Miracle/backend/model/cc.ko.300.bin"
    pos_tagger = Okt()
    model = g.fasttext.load_facebook_model(model_name)
    fastks = Keyword_similarity(model, 0.5, pos_tagger)
